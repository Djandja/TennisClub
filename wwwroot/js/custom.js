/* File Created: September 27, 2014 */
...
function onScroll(event) {
    try {
        ...
        if ($(window).scrollTop() + $(window).height() > $('#cbp-hrmenu').offset().top) {
        ...
        if ($(window).scrollTop() + $(window).height() > $('#next-match').offset().top) {
            setTimeout((function () { $('.bbtxt-box img').addClass('visible animated fadeInUp'); }), 1200);
...
/***   BOX SHOP CART   ***/
$(document).ready(function () {
    $('.box-login .fa-shopping-cart').click(function (e) {
        e.preventDefault();
